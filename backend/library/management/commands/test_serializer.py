...
# такой же как в models.py, только чисто под Python
class Author:
...
# базовый сериализатор для Author (должен содержать те же поля, что и в модели)
class AuthorSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        if attrs['last_name'] == 'Пушкин' and attrs['birthday_year'] != 1799:
            raise serializers.ValidationError('Год должен быть 1799')
        return attrs
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        biography = Biography('Бибография', author)
